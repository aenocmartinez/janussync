...
<div class="w-full px-4 sm:px-6 lg:px-8">
    ...
    <form method="GET" action="{{ route('scheduled-tasks.index') }}">
        <div class="relative mb-8 mt-6 max-w-md mx-auto sm:max-w-xl lg:max-w-3xl">
            <input type="text" id="taskSearch" name="search" value="{{ request('search') }}" placeholder="Buscar por nombre, estado o frecuencia..." class="w-full px-4 py-2 border border-gray-300 rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 pr-10 transition ease-in-out duration-150">
        ...
        <div class="block sm:hidden">
            @forelse($scheduledTasks as $task)
                <div class="p-4 border-b border-gray-200">
                    ...
                    <div class="text-gray-500">{{ ucfirst($task->status) }}</div>
                    <div class="text-gray-500">{{ $task->frequency }}</div>
                    <div class="text-gray-500">
                        @if($task->frequency == 'Personalizada')
                            {{ $task->custom_date }} {{ $task->custom_time }}
                        @else
                            N/A
                        @endif
                    </div>
        ...
        <table class="hidden sm:table min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Estado</th>
                    <th scope="col" class="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Frecuencia</th>
                    <th scope="col" class="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fecha/Hora Personalizada</th>
            ...
            <tbody id="taskTableBody" class="bg-white divide-y divide-gray-200">
                @forelse($scheduledTasks as $index => $task)
                    <tr class="hover:bg-gray-100 transition-colors duration-150">
                        <td class="py-4 px-6 text-sm text-gray-700">{{ $task->name }}</td>
                        <td class="py-4 px-6 text-sm">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $task->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">{{ ucfirst($task->status) }}</span>
                        </td>
    ...
    @if($scheduledTasks->hasPages())
        <x-paginator 
            :currentPage="$scheduledTasks->currentPage()" 
            :totalPages="$scheduledTasks->lastPage()" 
            :recordsPerPage="$scheduledTasks->perPage()" 
            :totalRecords="$scheduledTasks->total()" 
            :startRecord="$scheduledTasks->firstItem()" 
            :endRecord="$scheduledTasks->lastItem()" 
        />
    @endif
</div>

<!-- Componentes Modal para Confirmar Eliminación para cada tarea -->
@foreach($scheduledTasks as $index => $task)
    <x-modal-delete modalId="deleteTaskModal{{ $index + 1 }}" formId="deleteTaskForm{{ $index + 1 }}" />
@endforeach

<!-- Formularios de eliminación para cada tarea -->
@foreach($scheduledTasks as $index => $task)
    <form id="deleteTaskForm{{ $index + 1 }}" action="{{ route('scheduled-tasks.destroy', ['task' => $task->id]) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
@endforeach
...
<script>
// Función para filtrar la tabla de tareas
document.getElementById('taskSearch').addEventListener('keypress', function(event) {
    if (event.key === 'Enter') {
        event.preventDefault(); // Evita el envío automático
        this.form.submit(); // Envía el formulario solo al presionar Enter
    }
});
